...
*Mudd Labs | fedora-vm (KVM guest on dell-ubuntu) | Last Updated April 15th 2026*
...
| Field | Value |
|-------|-------|
| Host machine | `fedora-vm` (KVM guest on `dell-ubuntu`) |
| Tailscale IP | `<TAILSCALE-IP-T1>` |
...
> **Note:** The `:z` flag on the volume mounts handles SELinux relabeling automatically. Do not skip it — Fedora runs SELinux enforcing by default and the container will fail to start without it.
...
docker run -d \
  --name n8n \
  --network host \
  -e N8N_HOST=<TAILSCALE-IP-T1> \
  -e N8N_PROTOCOL=https \
  -e WEBHOOK_URL=<TAILSCALE-IP-T1> \
...
| `/usr/local/bin/muddbuilt` | `/scripts` | Exposes MuddBuilt scripts and device config to the container for the network monitor workflow |
...
| Tailscale (any mesh device) | `https://<TAILSCALE-IP-T1>` (via Tailscale Serve) |
...
Gitea runs bare metal on `fedora-vm`, the same guest as n8n. Webhooks must use HTTP on localhost — not HTTPS, not the Tailscale IP.
...
```
http://127.0.0.1:5678/webhook/<WEBHOOK-ID>
```
...
| Symptom | Cause | Fix |
|---------|-------|-----|
| Container crash-loops on startup with `EACCES` | SELinux blocking writes to host-mounted volume | Ensure `:z` flag is on all `-v` mounts |
...
| n8n data lost after container recreate | No volume mount on original container | Always use the docker run command above with all 3 `-v` mounts |

...
**Status:** Complete — exit node functional
...
# ssh -i ~/.ssh/muddcloud.key mudd@<ORACLE-PUBLIC-IP>
...
# Should return <ORACLE-PUBLIC-IP>
...
### SSH Lockout During Initial Setup

During initial setup the `ubuntu` user was expired before copying SSH keys to the `mudd` user, locking out all SSH access. All Oracle recovery paths (Bastion, Run Command, Console Connection) were exhausted without success.

**Resolution:** Instance was terminated and rebuilt. Current muddcloud instance was provisioned deliberately using the user setup sequence above.

Full incident detail: `troubleshooting/muddcloud-exit-node.md`

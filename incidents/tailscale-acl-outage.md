...
A secondary issue was discovered: during the outage, the Fedora VM dropped off Tailscale and required a daemon restart to pull the updated ACL.
...
| dell-ubuntu | tag:t1 | No internet |
| fedora-vm | tag:t1 | No internet + Tailscale went offline |
...
<TAILSCALE-IP-T2> dev tailscale0   ← Pi 4 (exit node)
```

Every device had `default dev tailscale0` — all internet traffic routing through the Pi 4. The ACL wasn't permitting that forwarding so traffic disappeared.
...
"grants": [
    ...
    // Parked: Desktop - no permissions until promoted
    // No rules implied - default deny
...
*Filed under: homelab/incidents/tailscale-acl-outage.md*
...
**Status:** Exit node resolved. SSH lockout unrecovered — original instance replaced.
...
# <TAILSCALE-IP-CLOUD> dev tailscale0 ← muddcloud peer present
```
...
- `<ORACLE-VCN-SUBNET>` — Oracle VCN subnet
- `<ORACLE-METADATA-IP>` — Oracle instance metadata service
...
| Scenario | Result |
|---|---|
| ThinkPad, no exit node | `<HOME-IPV6>` |
...
The `ubuntu` user was expired before copying its SSH authorized_keys to the `mudd` user. Both the new user (no keys) and the old user (expired) were inaccessible via SSH.
...
All recovery paths exhausted. Instance was replaced. A new muddcloud instance was provisioned and configured deliberately using the prevention steps below.
...
# ssh -i ~/.ssh/mudd-cloud.key mudd@<ORACLE-PUBLIC-IP>
...
4. Oracle internal routes appear as unapproved on every Oracle VM — never approve them.

...
Look for the LVM walkthrough segment — it demonstrates the full `pvcreate → vgcreate → lvcreate → mkfs → mount` chain live, which is the same sequence practiced in Task 4 of this week's lab script. Watch alongside or right before Session B.
...
`dell-ubuntu` is the one device in the fleet actually running LVM in production — its real `pvdisplay`/`vgdisplay`/`lvdisplay` output (from Task 3 of the lab) is worth screenshotting or noting down once, since it's the concrete reference point for every LVM question going forward. `tp-mudd` and `dell-fedora` default to xfs (Fedora), while `dell-ubuntu`, `muddpi`, and `pi-zero` default to ext4 — meaning the fleet already demonstrates the "xfs can't shrink, ext4 can" distinction as a live fact about specific machines, not an abstraction. The `dell-fedora` Fedora VM sits on top of whatever storage `dell-ubuntu`'s KVM/libvirt stack presents to it — worth tracing that chain down to the physical disk once LVM concepts solidify, since it connects this week directly into Week 3's virtualization material. Kernel module management is lower-profile in this homelab so far, but `wireguard`/`tailscale` module state on every Tailscale-connected node (all six devices) is a live example of `lsmod`/`modprobe` in daily use, even without noticing it as such.

...
Look for the networking-tools lab segment (`ip`, `ss`, `dig` demos) before Session B Task 1–3, and the `tar`/`rsync` walkthrough before Tasks 4–5. If there's a KVM/virt-manager segment, it maps to Task 6 — and if you take the Task 6g stretch goal, you'll build the same thing they demo, on this laptop.
...
This is the week the homelab stops being background and becomes the syllabus — and all of it is observable from this laptop's chair. Objective 1.4 describes the network `tp-mudd` itself sits in: its DNS path (`tp-mudd` → MagicDNS `100.100.100.100` → the tailnet's configured resolver) is a resolution chain you configured by hand within the last two weeks, and Lab Task 2 makes you observe it with the exam's tools (`resolvectl`, `dig @server`, `tcpdump`) plus *prove* the `nsswitch.conf` ordering by overriding a name in `/etc/hosts` and watching `files` beat `dns`. Task 3's insight is bigger than it looks: by running the server yourself (`python3 -m http.server`), every diagnostic tool's output becomes explainable because you control both ends — that's a habit worth keeping for every future networking topic. For 1.7, the key realization is that `tp-mudd` *is* a hypervisor: the same AMD-V flag you confirmed in Week 2 means the whole KVM/QEMU/libvirt stack — including snapshots and the destroy-vs-undefine distinction — can be practiced against a throwaway VM that costs nothing (the `dell-fedora` VM on the Dell remains a useful mental example of NAT-plus-Tailscale networking, but nothing requires touching it). The gap is 1.6: no scheduled backups exist yet. Task 5's rsync mirror of a local directory is the seed of a real one — pointing it at this laptop's `~/homelab` and wiring it to a systemd timer (Week 5's material) would close the only objective this week that the machine doesn't already demonstrate.
